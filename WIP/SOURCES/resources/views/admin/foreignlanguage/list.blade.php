                    ...
                    <div class="actions">
                        <div class="btn-group">
                            <button class="btn sbold green">
                                <a href="{{route('admin.foreignlanguage.form')}}" target="_blank" class="add-more-btn">
                                    THÊM HÌNH NGOẠI NGỮ <i class="fa fa-plus"></i>
                                </a>
                            </button>
                        </div>
                    </div>
                    ...
                    <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 85%;" class="text-center">Ngoại ngữ</th>
                                <th style="width: 61px;"></th>
                            </tr>
                        </thead>
                        <tbody>
                            @if(count($foreignlanguageList) > 0)
                                @foreach($foreignlanguageList as $index=>$item)
                                    <tr class="gradeX odd" role="row">
                                        <td class="text-center">{{ $index + 1 }}</td>
                                        <td class="text-center">{{ $item->name }}</td>
                                        <td>
                                            <a href="{{route('admin.foreignlanguage.form'). '?id=' . $item->id}}" target="_blank">
                                            ...
                                            <a class="sweet-delete"
                                                data-id="{{$item->id}}"
                                                data-url="{{route('admin.foreignlanguage.delete', ['id' => $item->id])}}">
                                                <button type="button" class="btn btn-icon-toggle " data-toggle="tooltip"
