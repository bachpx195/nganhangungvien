                    ...
                    <div class="actions">
                        <div class="btn-group">
                            <button class="btn sbold green">
                                <a href="{{route('admin.employmentstatus.form')}}" target="_blank" class="add-more-btn">
                                    THÊM HÌNH THỨC LÀM VIỆC <i class="fa fa-plus"></i>
                                </a>
                            </button>
                        </div>
                    </div>
                    ...
                    <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 85%;" class="text-center">Hình thức mà việc</th>
                                <th style="width: 61px;"></th>
                            </tr>
                        </thead>
                        <tbody>
                            @if(count($employmentstatusList) > 0)
                                @foreach($employmentstatusList as $index=>$item)
                                    <tr class="gradeX odd" role="row">
                                        <td class="text-center">{{ $index + 1 }}</td>
                                        <td class="text-center">{{ $item->name }}</td>
                                        <td>
                                            <a href="{{route('admin.employmentstatus.form'). '?id=' . $item->id}}" target="_blank">
                                            ...
                                            <a class="sweet-delete"
                                                data-id="{{$item->id}}"
                                                data-url="{{route('admin.employmentstatus.delete', ['id' => $item->id])}}">
                                                <button type="button" class="btn btn-icon-toggle " data-toggle="tooltip"
